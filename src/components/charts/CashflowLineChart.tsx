...
const SERIES_LABEL: Record<string, string> = { income: "Tiền vào", expense: "Chi tiêu" };

/**
 * Biến động thu chi — smooth Tiền vào (xanh) + Chi tiêu (cam) lines with gradient
 * area fills, matching the reference PFM design. No-data months are passed as null
 * so the lines break rather than dipping to a misleading 0đ (invariant #6).
 * Numbers come straight from the engine's `cashflowTrend` (invariant #1).
 */
    ...
    <ChartFrame height={240} label="Xu hướng thu chi theo tháng">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 24, right: 12, bottom: 0, left: 0 }}>
          ...
          />
          <Tooltip
            formatter={(v: number, name: string) => [formatVndUnit(v), SERIES_LABEL[name] ?? name]}
