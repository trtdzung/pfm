...
/**
 * Biến động chi tiêu — a smooth expense line with a gradient area fill, matching
 * the reference PFM design. No-data months are passed as null so the line breaks
 * rather than dipping to a misleading 0đ (invariant #6). Numbers come straight
 * from the engine's `cashflowTrend` (invariant #1). Income was removed.
 */
export function CashflowLineChart({ points }: { points: CashflowTrendPoint[] }) {
  ...
  return (
    <ChartFrame height={240} label="Xu hướng chi tiêu theo tháng">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 24, right: 12, bottom: 0, left: 0 }}>
          ...
          />
          <Tooltip
            formatter={(v: number) => [formatVndUnit(v), "Chi tiêu"]}
