...
export function HomeInsightWidget() {
  ...
  const ready = profile.ready && goals.ready && jars.loaded && corrections.loaded && !corrections.unsaved;
  const payload = JSON.stringify({ cif: persona.cif,
    profile: { assets: profile.assets, liabilities: profile.liabilities, goals: goals.goals,
      complete: !profile.loadFailed && !goals.loadFailed && !profile.dropped && !goals.dropped },
  ...
  async function dismiss() {
    if (!insight) return;
    try {
      const response = await fetch("/api/proactive-insights/current", { method: "POST",
        headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cif: persona.cif,
          snapshotId: insight.snapshotId, event: "dismissed" }) });
      if (!response.ok) throw new Error();
      setResult({ key: resultKey, insight: null });
      close();
    } catch { setError("Chưa ẩn được gợi ý. Vui lòng thử lại."); }
  }
  function chat() {
    if (!insight) return;
  ...
  return <>
    <section aria-label="Gợi ý tài chính" className="flex min-h-14 items-center rounded-2xl border border-orange-100 bg-white/95 px-2 shadow-card">
      ...
      <button type="button" aria-label="Ẩn insight" onClick={() => void dismiss()} className="flex h-11 w-11 shrink-0 items-center justify-center text-muted"><X size={16} /></button>
    </section>
    {error && <p role="alert" className="text-xs text-white">{error}</p>}
    {expanded && <Sheet title={insight.title} onClose={close} description={`Dữ liệu demo ngày ${insight.asOf}${insight.source === "fallback" ? " · Gợi ý từ dữ liệu" : " · M-Your"}`}>
