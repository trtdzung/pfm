 ...
 * — expense or income. Structural types (transfer / card_payment / fee / refund)
 * are NEVER labelled by the assistant (Red Team #4): they are excluded from
 * income/expense truth and map to no spending category, so the classifier must
 * not invent one for them. This is the single gate every trigger + validator
 * reuses, so eligibility can never diverge across the pipeline.
 */
...
export const AI_ELIGIBLE_TYPES: ReadonlySet<Transaction["type"]> = new Set(["expense", "income"]);
