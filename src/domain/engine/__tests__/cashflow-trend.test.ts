...
describe("cashflowTrend", () => {
  const txns = [
    txn({ postedAt: "2026-08-05T10:00:00.000Z", amount: 20_000_000, direction: "credit", type: "income", categoryId: "salary" }),
    txn({ postedAt: "2026-08-10T10:00:00.000Z", amount: 5_000_000, direction: "debit", type: "expense", categoryId: "dining" }),
    txn({ postedAt: "2026-09-05T10:00:00.000Z", amount: 22_000_000, direction: "credit", type: "income", categoryId: "salary" }),
  ...
  it("[H3] flags months that predate any data as hasData:false (gap, not 0 bar)", () => {
    ...
    expect(byMonth["2026-06"].income).toBe(0);
    expect(byMonth["2026-06"].expense).toBe(0);
    // Months with transactions → real data.
    expect(byMonth["2026-08"].hasData).toBe(true);
    expect(byMonth["2026-08"].income).toBe(20_000_000);
    expect(byMonth["2026-08"].net).toBe(15_000_000);
    expect(byMonth["2026-09"].hasData).toBe(true);
  });

  it("distinguishes a genuine-zero month from a no-data month", () => {
    // A month with only a reversed txn nets 0 but still has posted-record data?
    // Use an income-only month: expense genuinely 0, but hasData true.
    const incomeOnly = [txn({ postedAt: "2026-07-05T10:00:00.000Z", amount: 10_000_000, direction: "credit", type: "income", categoryId: "salary" })];
    const t = cashflowTrend(incomeOnly, "2026-07", 2);
