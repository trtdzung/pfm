...
describe("financialHealth", () => {
  it("computes both indicators with estimated provenance", () => {
    const nw = networth({
    ...
    const h = financialHealth(cashflow({ expense: 9_000_000, fixed: 3_000_000 }), ACCTS, nw);

    expect(h.runwayMonths.value).toBeCloseTo(63_000_000 / 9_000_000, 5);
    expect(h.concentration.value).toBeCloseTo(0.6, 5); // 60M / 100M
    for (const ind of [h.runwayMonths, h.concentration]) {
  ...
  it("[missing] concentration is null when there are no valued assets", () => {
    const h = financialHealth(cashflow({ expense: 5_000_000 }), ACCTS, networth({ assetsTotal: 0 }));
  ...
  it("[M4] concentration carries hasUnknown caveat when net worth has unvalued assets", () => {
    ...
    const h = financialHealth(cashflow({ expense: 8_000_000 }), ACCTS, nw);
