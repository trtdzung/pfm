...
describe("useAutoFund — C4: undo removes the fund then re-validates BOTH sides (never rests negative)", () => {
  it("undo genuinely resolves to 'undone' once the trigger's spend no longer overspends the jar", async () => {
    h.jarConfig = {
      version: 3,
      jars: [
        { id: "food", label: "Ăn uống", categoryIds: ["dining"], budgetLimit: 4_000_000, role: "spending" },
    ...
    // The overspend has since been resolved another way (e.g. the trigger's amount
    // was corrected down) — undo must NOT reapply a now-unnecessary cover.
    act(() => {
      result.current.updateTxn(triggerId, { amount: 3_000_000 });
    });
    let outcome: ReturnType<typeof result.current.undo> | undefined;
    act(() => {
      outcome = result.current.undo({ triggerTxnId: triggerId, targetJarId: "food", postedAt: "2026-09-05T10:00:00.000Z" });
    });
    expect(outcome?.status).toBe("undone");
    expect(rebalancesFor(result.current.manualTxns, triggerId)).toHaveLength(0);
  });

  it("undo REAPPLIES a fresh cover when the trigger's spend still overspends the jar (documented re-validate contract)", async () => {
    h.jarConfig = {
      version: 3,
      jars: [
        { id: "food", label: "Ăn uống", categoryIds: ["dining"], budgetLimit: 4_000_000, role: "spending" },
    ...
    // The spend itself is unchanged (still 500k over) — removing coverage
    // re-exposes the SAME shortfall, so undo re-funds it rather than leaving the
    // jar negative (the invariant: no jar ends over-budget-unfunded).
    let outcome: ReturnType<typeof result.current.undo> | undefined;
    act(() => {
      outcome = result.current.undo({ triggerTxnId: triggerId, targetJarId: "food", postedAt: "2026-09-05T10:00:00.000Z" });
    });
    expect(outcome?.status).toBe("reapplied");
    expect(rebalancesFor(result.current.manualTxns, triggerId)).toHaveLength(1); // still exactly one, never zero
  });

  it("undo leaves the durable 'residual' (C5) state when the re-exposed shortfall can't be auto-covered", async () => {
    h.jarConfig = {
      version: 3,
      jars: [
        { id: "food", label: "Ăn uống", categoryIds: ["dining"], budgetLimit: 4_000_000, role: "spending" },
    ...
    // Seed the "already funded via a confirmed goal raid" precondition via `commit()`
    // directly (NOT `fundJar(..., includeGoal: true)`): `fundJar` can never reach
    // `commit()` for a requiresManualGoal case (see the REGRESSION comment on the
    // `an explicit confirm (includeGoal) commits...` test above) — this test is about
    // `undo`'s own re-validate logic, so it bypasses that unrelated, already-reported
    // bug rather than being blocked by it.
    act(() => {
      result.current.commit({
        assessment: {
          ...
          shortfall: 800_000,
          donors: [],
          goalDonors: [{ jarId: "goal", label: "Hũ Mục tiêu", take: 800_000 }],
          requiresManualGoal: true,
          targetJarId: "food",
          source: "mock",
        },
        targetJarId: "food",
    ...
    // Re-exposed shortfall (800k) is once again only coverable by the protected
    // `goal` jar — undo does NOT silently re-raid it; the jar is left in the
    // durable "cần bù thủ công" state instead.
    expect(outcome?.status).toBe("residual");
    expect(rebalancesFor(result.current.manualTxns, triggerId)).toHaveLength(0);
  });
});

describe("useAutoFund — H1: 'Đổi nguồn' (changeSource) is atomic — the new leg replaces the old, never both/neither", () => {
  it("swaps the donor: the new leg fully covers, the old leg is gone, exactly one active rebalance remains", async () => {
    h.jarConfig = {
      version: 3,
      jars: [
        { id: "food", label: "Ăn uống", categoryIds: ["dining"], budgetLimit: 4_000_000, role: "spending" },
    ...
    h.accounts = [account("cur", 11_000_000)];
    ...
    let fundedIds: string[] = [];
    act(() => {
      const r = result.current.fundJar({ targetJarId: "food", triggerTxnId: triggerId, postedAt: "2026-09-05T10:00:00.000Z", origin: "auto" });
      fundedIds = r.createdIds;
    });
    expect(rebalancesFor(result.current.manualTxns, triggerId)).toEqual([expect.objectContaining({ rebalance: expect.objectContaining({ fromJarId: "buf" }) })]);

    act(() => {
      result.current.changeSource({ triggerTxnId: triggerId, targetJarId: "food", postedAt: "2026-09-05T10:00:00.000Z", oldIds: fundedIds, donorJarId: "ess" });
    });

    const rebs = rebalancesFor(result.current.manualTxns, triggerId);
    expect(rebs).toHaveLength(1); // never zero (unfunded), never two (double-funded)
    expect(rebs[0]).toMatchObject({ amount: 500_000 });
    expect(rebs[0].rebalance).toMatchObject({ fromJarId: "ess", toJarId: "food", origin: "manual" });
  });

  it("refuses the swap (keeps the old leg intact) when the chosen donor can't fully cover the shortfall", async () => {
    h.jarConfig = {
      version: 3,
      jars: [
        { id: "food", label: "Ăn uống", categoryIds: ["dining"], budgetLimit: 4_000_000, role: "spending" },
        { id: "buf", label: "Dự phòng", categoryIds: ["buffer-cat"], budgetLimit: 6_000_000, role: "buffer" },
        { id: "tiny", label: "Nhỏ", categoryIds: ["misc"], budgetLimit: 100_000, role: "essential" },
      ],
    };
    h.accounts = [account("cur", 6_100_000)];
