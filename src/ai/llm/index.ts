 ...
 *
 * Provider resolution:
 *  - `LLM_PROVIDER` explicit wins ("vng" | "none"/"off" | "anthropic").
 *  - Unset: infer "vng" when `AI_PLATFORM_API_KEY` is present, else "anthropic".
 *
 * Only the VNG GreenNode (OpenAI-compatible) adapter is implemented in this
 * prototype slice; "anthropic" is documented but has no adapter yet, so it
 * resolves to `null` (offline) until one is added — one file, no pipeline change.
 */
...
export function getLlmClient(): LlmClient | null {
  ...
  if (process.env.AI_PLATFORM_API_KEY) {
    console.warn(
