...
function request(origin = "http://localhost:3000") {
  return new NextRequest("http://localhost:3000/api/stt/session", { method: "POST", headers: { origin } });
}
beforeEach(() => {
...
describe("STT ticket proxy", () => {
  it("keeps the service key on the server and returns a WSS URL plus ticket", async () => {
    ...
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toEqual({ origin: "http://localhost:3000" });
