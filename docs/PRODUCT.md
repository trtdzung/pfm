...
- **A jar is a group of expense categories with two numbers: a monthly Hạn mức and a running Số dư** (see "Hạn mức và Số dư" below). Neither moves real money on its own; setting the limit or putting money into a jar is pure bookkeeping of money already in the user's CASA account (invariant #3).
- **Every expense category belongs to AT MOST one jar.** A category in no jar (its jar was deleted, or it was just created) is legal — "chưa xếp hũ": nothing is invented to hold it, its spend still counts in totals and the reports group it under **"Chưa xếp hũ"** rather than silently dropping it, and it can be assigned to any jar at any time.
...
- **Setup and edits live in Cài đặt → "Hũ & danh mục":** create/edit/delete a jar (name, color, icon, monthly limit — deleting a jar deletes it outright — its categories become "chưa xếp hũ", their past spend still counts under "Chưa xếp hũ") and reassign a category to a different jar (at-most-one is enforced: assigning drops it from the old jar automatically). The jar editor also has a full add/remove category picker (every expense category listed, checked ones tinted, others labelled `đang ở <hũ>` — a tap only changes an unsaved draft — the picks are saved in ONE write when the editor is closed, so picking a category from another jar and unpicking it again changes nothing). **The category taxonomy is user-editable, per persona:** create a category (from the jar editor, straight into that jar, or from the transaction category picker, landing unassigned ("Chưa xếp hũ")), rename it, hide it, or delete it. Preset categories stay locked (cannot be renamed, hidden, or deleted); a duplicate jar/category label is flagged with a warning. A custom category already in use by a transaction cannot be hard-deleted — the delete flow offers **hiding (archiving)** instead, which keeps the category's jar membership so historical jar/spend totals never move; an unused custom category deletes outright.
...
- **Hạn mức và Số dư — two different numbers (implemented, plan `260923-jar-limit-vs-balance-split`).** This replaced the earlier "one number per jar" model, where the limit was also the jar's balance and the balance silently reset to the limit every month.
  - **Hạn mức** (limit) is the monthly plan: "Đã chi X / hạn mức Y" for the selected month, with the "Vượt hạn mức" and 80% warnings. It starts again every month. Changing it never changes the money in the jar.
  - **Số dư** (balance) is the money in the jar: what the user put in (số dư ban đầu, "Nạp vào hũ", "Chia ngay") minus what they took back ("Rút về Chờ phân bổ") minus what the jar's categories spent since the jar was created, plus/minus inter-jar coverage ("điều chỉnh hũ"). It **carries over** from month to month: a jar that spent 3tr of a 7tr deposit in September opens October with 4tr, not with its limit.
  - The two can disagree, and both are shown: a jar can be over its plan and still have money ("Đã vượt hạn mức" + a positive Số dư), or within its plan and out of money. A Số dư below 0 is shown as negative (red) with "Cần bù thủ công" / auto-fund; only the spendable amount used for transfers floors at 0.
  - **Unknown stays unknown.** A jar nobody has put money into yet (e.g. a jar added by applying a template) reads **"Chưa có số dư"**, never 0₫, and offers "Chia ngay" in the current month. For a month **before the jar existed**, Số dư is also "Chưa có số dư", while "Đã chi / hạn mức" for that month still shows.
  - **Creating a jar needs both numbers:** a monthly hạn mức and a **Số dư ban đầu** (0 is allowed and means a known empty jar). The AI assistant's jar-create card asks for both too, and the user confirms it; the assistant never deposits or withdraws on its own.
- **"Hũ chi tiêu" row trên Tổng quan** — a horizontally-scrolling row on the Tổng quan cockpit. A **"Chờ phân bổ"** card shows the CASA money (Σ `availableBalance` of `type:"current"` accounts) that no jar's Số dư holds yet (`CASA − Σ max(0, số dư)`; unknown when there is no current account, never a fabricated 0). It is shown only for the current month (it is today's money, not a past month's) and hidden when there is nothing left. Its **"Chia ngay →"** sheet opens with "Còn lại để chia" equal to that same amount; each jar row starts at 0 and is the amount to **add** to that jar's Số dư, and saving puts it in all jars at once (all or nothing) — it never changes a hạn mức. Each jar card shows its **Số dư**. This is **pure bookkeeping of money already sitting in the user's accounts**: no transfer, no OTP, no money movement (invariant #3) — the sheet says "Không chuyển tiền, không cần OTP". The Chuyển-tiền "spendable" a jar shows as a transfer source is `max(0, Số dư)`, and the card, the sheet, the transfer picker's "Chưa phân bổ" and the server's cap all read the same pool number, so they can never drift.
- **Chuyển giữa các hũ (implemented, plan `260923-jar-to-jar-transfer-sheet`)** — tapping a jar card in this row, current month only, opens a "Chuyển giữa các hũ" sheet that moves **SỐ DƯ** (never a hạn mức) from one source to one destination — either endpoint can be a jar or the "Chưa phân bổ" pool. The source defaults to the tapped jar; a ⇅ button swaps source and destination. The amount is any positive whole VND up to the source's spendable cap — `max(0, Số dư)` for a jar, `max(0, phần chưa phân bổ)` for the pool — and a jar with no Số dư yet can neither give nor receive. A before→after preview of both sides shows before confirming. Like the rest of "Hũ chi tiêu" above, this is pure bookkeeping: no real money moves, no OTP, and hạn mức/đã chi/vượt hạn mức are untouched. A past month's jar cards are view-only — tapping does nothing (the earlier behavior of navigating to Ngân sách on tap was removed). On success, a status banner ("Đã chuyển X từ A sang B") confirms the move.
- **"Chưa gắn nhãn" card in the same row** — a data-quality prompt, not a jar: it shows this month's posted CASA money that never got a spending category — both uncategorised expenses and app-made transfers still tagged "Chuyển khoản" (count + amount) — and appears even when no jars exist. Tapping it opens a sheet listing those transactions; picking a category labels each one and its spend now counts toward that category's jar. A transfer only becomes a spend on this explicit choice (it is flipped transfer→expense); until then it is excluded from spend truth (invariant #6). Bank-history transfers are not surfaced here — only ones the user recorded through the app.
- **Hard cap: jars can never hold more than the CASA balance.** Putting money into jars (creating a jar with a Số dư ban đầu, "Chia ngay", "Nạp vào hũ") is rejected on the server when it would make Σ spendable Số dư exceed the CASA balance, with the amount it is over by; taking money out can never exceed a jar's Số dư. Client-side warnings are UX only. A hạn mức is a plan, not money, so it is never capped by the CASA balance. Replacing all jars with a template asks for confirmation first when it would delete jars that still hold money.
- **Not a revival of the retired balance-lens residual, and not the orphaned surplus engine below.** The limit-and-balance model is neither a resurrection of the "Model A" balance-partition jar retired above, nor a rewiring of the orphaned `simulateSurplusAllocation`/`surplus.ts` engine described in the next bullet, which remains untouched and still has no mount point.
- **Every Số dư reading is the same number.** The Ngân sách card's "Số dư" line, the Tổng quan jar card and the Cài đặt jar editor all show the same engine-derived balance, and every "Đã chi / hạn mức" reading is the same plan-axis pair.
...
- **"Chưa phân bổ" is a virtual, derived transfer source**, the same number as the "Chờ phân bổ" card on Tổng quan: `casaBalance` (Σ `availableBalance` of `type:"current"` accounts) minus Σ every jar's derived `spendable` (`max(0, Số dư)`, `null`/non-fundable when the jar has no Số dư yet). It is never stored — no new field, no drift (invariant #1). When the user doesn't pick a jar as the transfer source, this pool is the default/selectable source; a no-jar transfer simply debits the CASA account, and the derived pool self-shrinks on the next read.
...
- **Server-enforced ceiling.** Independent of what the client proposes, the jar API rejects any write that would make Σ spendable Số dư exceed the CASA balance (`fitsCasaCap`) — the true source of truth for "did this allocation actually fit" lives server-side, not in the popup's suggestion. A jar carries no stored balance: Số dư is derived by the engine from the user's deposits/withdrawals and the jar's spend, and `spendable = max(0, Số dư)`.
...
- **Giao dịch** (`?tab=transactions`) — a PFM-scoped transaction list grouped by day with category chips; unclassified transactions are highlighted. Tapping a transaction opens its detail: category picker (always with a jar, since every category belongs to exactly one jar, and with a "＋ Thêm danh mục" option to create a new category on the spot, landing unassigned ("Chưa xếp hũ")), and a "hide from report" toggle.
...
- **Cài đặt** (`?tab=settings`) — "Hũ & danh mục": jar CRUD, a jar-level add/remove category picker, category CRUD (create/rename/hide/delete, presets locked), and category-to-jar reassignment — described under "Hũ chi tiêu" above. This is scoped to jars/categories only; it is **not** the same screen as the app-level `/settings` (consent, persona switcher, About — see below), which keeps its separate route and entry point.
...
3. **PFM (`/pfm`):** single route, 4-tab bottom nav + center FAB (`plans/260910-1626-pfm-bidv-wallet-reformat/`). First run shows a one-screen onboarding (template picker + tracked-account review) instead — see "Hũ chi tiêu" above.
   ...
   - **Giao dịch:** a day-grouped transaction list with category chips (unclassified transactions highlighted); tap-through to a detail screen with a category picker (always with a jar, plus a "＋ Thêm danh mục" option to create a category on the spot) and a "hide from report" toggle.
   ...
   - **Cài đặt:** "Hũ & danh mục" — jar CRUD (name/color/icon/monthly limit/delete), a jar-level add/remove category picker, category CRUD (create/rename/hide/delete; presets locked), and category-to-jar reassignment — see "Hũ chi tiêu" above. Distinct from the app-level Settings screen (item 5 below).
