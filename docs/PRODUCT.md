...
- **Server-enforced ceiling.** Independent of what the client proposes, the jar API rejects any write that would make Σ jar `actualAmount` exceed the CASA balance, or that would leave any jar with a negative balance — the true source of truth for "did this reallocation actually fit" lives server-side, not in the popup's suggestion.
