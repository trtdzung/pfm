...
- **Exactly-one category-to-jar mapping.** `categoryToJarMap`/`assignCategory` (`src/domain/engine/category-jars.ts`, `src/state/jars.tsx`) enforce that every expense category belongs to exactly one jar; unassigning is disallowed (moving a category to a new jar auto-drops it from the old one), and an orphaned category (stale config, bad migration) heals into a catch-all **"Khác"** jar (`KHAC_JAR_ID`/`KHAC_JAR_LABEL`) as a defensive fallback so totals never silently drop a category.
...
- `JarConfig` is now **`version: 3`** (`src/domain/models/index.ts`); `Jar.allocation`/`JarAllocation` (the balance-lens earmark fields) were dropped entirely. A stored v2 config **migrates forward** at the provider boundary (`migrateStoredJarConfig` in `mock-provider.ts`: strip `allocation`, keep any `budgetLimit`/`color`/`icon` already set, bump to v3) rather than being wiped; v1 or structurally invalid records reseed from the default template. `isValidJarConfig` is now a pure v3 guard; `state/jars.tsx`'s `migrateJarConfig` only dedupes/heals orphans against the already-v3 shape.
...
- **Persistence:** `Providers.getJarConfig()` / `saveJarConfig()` (`src/providers/interfaces.ts`) are unchanged as the only *write* methods on `Providers` — the model change is internal to `JarConfig`'s shape, not the interface. The mock adapter still backs them with persona-scoped `localStorage` (key `msb-pfm.jars.<personaId>`); see the v2→v3 migration above. A real adapter maps these two methods to the MSB preferences API (invariant #4) without changing the interface.
