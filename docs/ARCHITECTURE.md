...
**This model retired the earlier "Model A" balance-lens partition** (a jar as a `%`/fixed-VND snapshot slice of the current account balance, `Σ earmarks + "Chưa phân bổ" ≡ số dư`), which shipped for one release and is now fully removed — code, components, and tests (`src/domain/engine/jars.ts`, `AllocationMeter`, `SurplusPanel`, `JarList`, `JarCard`, `HuTab`, and their tests were deleted in `plans/260910-1626-pfm-bidv-wallet-reformat/`, phase 08). The shipped model instead defines **a jar as a group of expense categories with a monthly `budgetLimit` (hạn mức, the plan) and a derived running balance (số dư)** backed by a `jar_ledger` display partition of CASA — it never moves money (see "Limit vs balance" below).

`evaluateJarBudget(config, txns, period, prevPeriod, now, rebalanceNetByJar?, balances?)` (`src/domain/engine/jar-budget.ts`) is the plan-axis engine (the balance comes in precomputed from `jarBalances`, below) — named to avoid colliding with the pre-existing per-category `budget.ts`:

- **At-most-one category-to-jar mapping.** `categoryToJarMap` (`src/domain/engine/category-jars.ts`) and the invariant functions in `src/domain/jar-rules.ts` (`stripCategories`/`dedupeCategories`, used server-side by every `/api/jars*` route handler — see "Persistence" below) enforce that every expense category belongs to at most one jar (moving a category to a new jar auto-drops it from the old one). A category in no jar — a deleted jar's categories, a new category, an unchecked pick — is legal ("chưa xếp hũ"); nothing is synthesised to hold it (the old read-time `healOrphanCategories` → "Khác" jar was removed 2026-09-24), and totals never silently drop it: `groupSpendingByJar` reports its spend under a **"Chưa xếp hũ"** group (`KHAC_JAR_ID = "khac"` / `KHAC_JAR_LABEL`, a report group only — never a jar row). `src/state/jars.tsx` itself holds none of this logic anymore — it's a thin client (see below).
...
- **A limit stays unknown until set — never coerced to 0 (invariant #6).** `jar.budgetLimit === undefined` → `limitState: "unset"`, and `pct`/`status` are `null` (the balance is independent of the limit); the gauge's `totalLimit`/`pctUsed` are computed only over jars with a set limit (`setLines`), so an unset jar can never masquerade as a 0%/"ok" bar. `status` is `"over" | "near" (≥80%) | "ok"` for a set limit; `thresholdHit` flags the 80% line.
- **Two independent axes (plan `260920-1317-jar-transfer-balance-not-limit`).** `pct`/`status`/`thresholdHit` are always measured against the jar's ORIGINAL `limit` — an inter-jar rebalance never raises or lowers it, so `status === "over"` is exactly `spent > limit` regardless of any transfer coverage. `balance` (below) is the separate BALANCE axis: it folds deposits, withdrawals and the rebalance net, `pct`/`status` do not. A jar can be simultaneously "Đã vượt hạn mức" (plan axis, `status: "over"`) and topped back up ("Đã bù", balance axis, `balance >= 0`) — the UI shows both, never lets one silence the other. There is no `effectiveLimit`; that intermediate (rebalance-adjusted ceiling) existed briefly and was removed because it let a transfer rewrite the user's own plan.
...
- **Persistence:** `Providers` (`src/providers/interfaces.ts`) exposes `getJarConfig()` (read, including the jar ledger) plus six mutators — `createJar(jar, balance)`/`updateJar`/`postJarLedger(entries)`/`removeJar`/`assignCategory`/`replaceJars` — each returning the full, freshly-persisted `JarConfig` so the client never has to recompute or re-fetch. The mock adapter (`src/providers/mock/mock-provider.ts`) backs every one of these with a `fetch()` call to `/api/jars*` or `/api/jar-ledger` (SQLite-backed, see "Spending jars" above) rather than `localStorage` — jars were the first (and so far only) domain to move off the shared `userRecordStore<T>()` localStorage mechanism the asset/liability/goal CRUD pairs still use. A real adapter maps the same seven methods to the MSB preferences API (invariant #4) without changing the interface.
...
- **Insight — sole budget warning:** `src/insights/detectors/jar-pressure.ts` now reads `Financials.jarBudget.lines` (not the retired `jarPartition`) and warns on the worst `status: "over"`/`"near"` jar among those with a set limit — **skipping an `over` jar whose balance was already topped back up by a covering rebalance** (`coversByJar`, shared with `jarOverspendCovered`); severity/ranking read the balance (out of money → `urgent`, over limit but still funded → `attention`) and the fact reads "Số dư" (a `null` balance emits no balance fact), because `jar-overspend-covered` is the sole narrator for that case and the two must not tell the same story twice (H3); it is the **only** budgeting insight the product surfaces — the pre-existing per-category `budgetPressure` detector was removed from the detector registry (`src/insights/run.ts`) in the same change to avoid double-warning the same overspend from two systems. The per-category engine itself (`budget.ts`/`evaluateBudget`/`Financials.budgetLines`/`Providers.getBudgets`) is **kept**, deliberately, as AI-facing structured data — ripping it out would touch the AI facade's `load-financials` path and ~8 tests for no product value (YAGNI).
- **Limit vs balance — two numbers per jar, implemented (plan `260923-jar-limit-vs-balance-split`).** This replaced the earlier "Hũ một con số" (single-number) model, in which `budgetLimit` meant allocation, spending ceiling and starting balance all at once and the "balance" silently reset to the limit every month. A jar now carries two independent numbers on two axes:
  - **Hạn mức (limit) — the monthly PLAN (a flow).** `jars.budget_limit`, edited only by the user. `spent` = net expense of the jar's owned categories in the viewed period (`netExpenseByCategory` + first-owner rule); `overLimit`/`status`/`pct` compare `spent` with the limit and reset every month. A limit edit never moves money and is never CASA-capped.
  - **Số dư (balance) — money in the jar (a stock that carries over months).** Derived only by `jarBalances(config, txns, asOf, txnsTo)` (`src/domain/engine/jar-balance.ts`, invariant #1):
    `balance = Σ deposit − Σ withdraw − Σ spent[anchor, txnsTo] + Σ rebalanceNet[anchor, txnsTo]`.
    Ledger rows count when `createdAt ≤ asOf`; `anchor = jar.createdAt` (fallback: earliest ledger row); `asOf = balanceAsOf(period, now) = min(period.to, now)`; `txnsTo = period.to` (a posting is bank truth already in CASA and in `spent`, so a same-day posting stamped after the demo clock's time-of-day still counts). Spend, refunds, reversed and pending rules are exactly those of `spent`. No ledger row `≤ asOf`, or `asOf` before the anchor (a month before the jar existed) → `balance: null` ("Chưa có số dư"), never 0 (invariant #6) — while "Đã chi / hạn mức" for that month still shows. A 0 opening deposit is a known 0. A funded balance carries `source: self_reported` (invariant #5). The balance can go negative (spent past the money in the jar); it is shown as such and triggers the auto-fund / "Cần bù thủ công" paths.
  - **One clock (Red Team #1).** Every balance read and every write uses the same injected `now = transferNow()` (`src/lib/demo-clock.ts`: `DEMO_NOW`'s day + the real time-of-day) — `computeFinancials(…, { now })` from `useFinancials` and `/api/jar-summary`, `jarSpendableTotal` in the write guards, the ledger row stamps, and `useAutoFund`'s snapshot deps (the auto-fund snapshot reads balances as of the trigger's `postedAt`, itself stamped with `transferNow()`). `DEMO_NOW` (00:00) never feeds a balance path, so a deposit made a moment ago is never hidden. Month rollover is verified only by injected-clock tests (`jar-balance.test.ts` case D, `auto-fund-core.test.ts`), because the running app is pinned to 2026-09.
  - **Spendable and the pool (D26).** `spendable = jarSpendable(balance) = max(0, balance)` (`null` when the balance is unknown). `evaluateJarEnvelope(config, accounts, spentByJar, period, balances)` returns per-jar `{balance, limit, spent, overLimit, …}` lines and `pending = { pool: CASA, allocated: Σ spendable, amount: CASA − Σ spendable, overAllocated }`. The same number is `Financials.unallocatedPool` (`computeUnallocatedPool`), the transfer picker's "Chưa phân bổ", the AllocationSheet's opening "Còn lại để chia" and the server cap's headroom (`jarSpendableTotal`, `src/domain/engine/jar-spendable-total.ts`) — pinned by `src/domain/engine/__tests__/jar-pool-parity.test.ts` after deposit, withdraw, spend and rebalance. "Chờ phân bổ" is a current stock, so it is shown only for the current month.
  - **Ledger (`jar_ledger`, `src/lib/jar-ledger-store.ts`).** Rows `{id, jarId, kind: deposit|withdraw, amount, isOpening, createdAt, source: self_reported}`, read with the config (`JarConfig.ledger`, read-only on the wire). The ledger is a display partition of CASA — no money moves (invariant #3), and inter-jar moves stay `dieu-chinh-hu` REBALANCE transactions (below). Ids, stamps, `is_opening` and `source` are server-minted; the client never sets them.
  - **Write doors.** `POST /api/jars {cif, jar, balance}` — the opening `balance` is REQUIRED whole VND in `[0, 10^12]` (0 allowed) while `jar.budgetLimit` is OPTIONAL (omitted/null = "chưa đặt", stored NULL); the jar row and its opening row (`is_opening = 1`, also for 0) are written in one transaction. `POST /api/jar-ledger {cif, entries}` (`src/app/api/jar-ledger/route.ts`, a sibling of `/api/jar-summary`) is the ONE door for deposits/withdrawals: 1–50 entries, each amount in `(0, 10^12]`, Σ ≤ 10^12, one SQLite transaction (all-or-nothing); 404 for an unknown jar, 422 for a jar with no DB row (the healed "Khác"), a withdraw over the balance as re-derived through the batch so far (`maxWithdraw`), or the cap. `PUT /api/jars` (template apply / reset) writes NO ledger rows — a new id's balance is `null` until the user deposits — and deletes the ledger of dropped ids; the client asks for confirmation first when the replace would delete jars that hold a balance (`jarsLosingBalance`, `{ confirmedBalanceLoss }`). The batch `PATCH /api/jars` and client `updateJars` were removed (405); there is no per-jar ledger route and no reserved `ledger` jar id.
  - **Cap (`fitsCasaCap(nextSpendable, baseSpendable, casaPool)`, `src/domain/engine/allocation-plan.ts`; server `capViolation` in `src/app/api/jars/jar-write-guards.ts`).** A write fails with `422 { error: "over CASA cap", overBy }` only when it RAISES Σ spendable over the stored config AND the new Σ exceeds CASA (`casaPoolForCif`, the live Σ `availableBalance` of `current` accounts; an unknown CASA fails). Checked on create (with the unwritten opening deposit folded in), on each ledger batch (once, on the whole batch — a withdraw-A/deposit-B redistribution passes when its net fits), on `PUT`, and on a `PATCH /api/jars/:id` only when `categoryIds` change (a move re-attributes spend since the anchor). Never on a limit edit.
  - **Migration + backup (`src/lib/db-migrate-jar-ledger.ts`, wired into `getDb()`).** When `jars` rows lack `created_at`, the server first copies the DB to `data/pfm.sqlite3.bak-<YYYYMMDD-HHmmss>` (a failed copy aborts), then adds the column and, in one transaction, sets `created_at` = start of the current demo month and gives every jar with a non-NULL `budget_limit` (0 included) one opening deposit of that amount (`led-mig-<jarId>`); a NULL-limit jar gets none. So current-month numbers equal the pre-split ones. `scripts/seed-db.mjs` seeds the same shape. Restore path: `data/schema.md` → "Migration and restoring a pre-migration backup".
  - **Client.** `src/state/jars.tsx`: `addJar(jar, balance)`, `postLedger(entries)` (the one balance mutator), `applyTemplate`/`resetToSeed` with the balance-loss guard. The session-only top-up overlay (`state/jar-topup.tsx`, `useJarTopup`) and `state/use-casa-pool.ts` were deleted: top-ups now persist as ledger rows, and every screen reads the pool from `useFinancials`/`useCurrentJarFunds`.
  - **UI.** Tổng quan (`src/components/hu-envelope/`): `PendingAllocationCard` ("Chờ phân bổ", current month only, hidden when ≤ 0) + one `JarEnvelopeCard` per jar showing "Số dư". Tapping a jar card (current month only) opens `JarTransferSheet` — see "Jar-to-jar balance transfer" below — instead of "Chia ngay →", which opens `AllocationSheet`: an ADD-TO-JAR deposit sheet — "Còn lại để chia" opens at `pending.amount`, each row opens at 0 and is the amount to add, and saving posts ONE `postLedger` batch of deposits (never a limit change). Ngân sách (`HuBudgetCard` + `JarBalanceRow`): an "Đã chi X / hạn mức Y" bar (plan axis) and a separate "Số dư" line (balance axis; negative shown in `text-negative`; "Chưa có số dư" + "Chia ngay" for an unfunded jar in the current month). Cài đặt: `HuCreateSheet` requires both the hạn mức and "Số dư ban đầu"; `HuEditorSheet` edits the limit (`HuLimitField`) and deposits/withdraws (`HuBalanceField`, "Nạp vào hũ" / "Rút về Chờ phân bổ").
  - **Agent contract (`backend_docs/pfm-read-api.md`).** `create_jar` requires `initial_balance` (≥ 0) and the card creates through `addJar(jar, balance)` after the human confirms; `/api/jar-summary` returns `limit` and `balance` per jar, keeps `remaining` as a DEPRECATED alias of `balance`, and `allocationHeadroom` = "Chờ phân bổ". The agent never deposits or withdraws.
  - **Accepted risks.** A category re-mapping re-attributes past spend since the anchor, so a balance changes retroactively (prototype). The pool can go negative when CASA is spent outside jars (`overAllocated`). There is no server-side cif/session binding (prototype-wide), so a `422 maxWithdraw` body reveals a jar's balance to any caller that knows the cif.
- **`jar-budget.ts` and `jar-envelope.ts` remain two parallel engines by design (tracked as M7).** Ngân sách reads `jar-budget.ts`; Tổng quan's envelope row reads `jar-envelope.ts`; both read the same `jar.budgetLimit` and the same `jarBalances` map (computed once in `computeFinancials`), so they can never disagree on either number, but they are not merged into one engine. Unifying them is unscoped future work.
- **Level 3 surplus allocation remains orphaned — unrelated to the jar model above.** `src/domain/engine/surplus.ts` (`surplusFromResidual`, `simulateSurplusAllocation`) is untouched code and its tests still pass, but its sole valid input — the long-retired balance-partition's "Chưa phân bổ" residual — no longer exists in this model, and `SurplusPanel` (its only UI) was deleted with the rest of the balance-lens component tree. No component currently imports `surplus.ts`; it has no data source and no mount point. Re-deriving "surplus" against the current model (e.g. from `cashflow.net`) is unscoped future work, not a shipped capability.
...
Pure engine pieces feeding the Chuyển tiền (transfer) flow and every overspend-coverage path. Together with `jar-rebalance.ts` (see "Limit vs balance" above) they are the current, shipped "hũ không-thể-âm + auto-rebalance" model. None introduces a new stored field — every number is derived from the jar balances (`jar_ledger` + `spent` since the anchor + the rebalance legs already persisted as self-reported rows of `transactions`).
...
- **Retired: "spread-as-spend."** The earlier top-up mechanic wrote one **self-reported expense transaction per contributing jar, charged into that jar's own real category** (`addManualTxn` per donor into `categoryIds[0]`) — including the source jar's own portion — so recording the spend against each jar's category was what drew its derived balance down. This inflated spend-by-category for every donor even though the money was never actually spent in that category. It has been fully replaced by the rebalance-as-Transaction model above: the real spend keeps its real category (spend-by-category is honest), and inter-jar coverage moves only the jar `balance` via the separate `dieu-chinh-hu`-tagged leg(s). The earlier idea of a dedicated `JarRebalance`/`jar_rebalances` ledger table was also dropped in favor of riding the existing self-reported rows of `transactions` — no new table.
- **Over-allocated residual, never silently zeroed (invariant #6).** `jarNeedsManualTopUp(balance)` (`src/domain/engine/jar-spendable.ts`) is the durable per-jar "cần bù thủ công" condition — `balance < 0` with no covering rebalance; it is re-derived every render (never a stored flag), so it can't drift and keeps re-surfacing until the jar is funded or the over-allocation is accepted. The sole insight for this (`src/insights/detectors/jar-overspend-covered.ts`) reports it two ways per jar, once per period: **COVERED** ("đã bù … từ …", with each donor's `origin` in the copy) when a rebalance this period funded the jar, or the **RESIDUAL** ("cần bù thủ công") when it is still unfunded — never double-alarming the same overspend as the separate `jarPressure` budget-limit warning. At the pool level, the same story is `computeUnallocatedPool`'s `overAllocated` flag (Σ spendable over CASA — e.g. CASA spent outside jars — or an overspend beyond every donor's reach) — "Vượt phân bổ".
- **Server-side ceilings.** Σ spendable ≤ CASA (`fitsCasaCap`, see "Cap" under "Limit vs balance" above) on create, ledger batch, `PUT` and category moves, plus withdraw ≤ balance on the ledger batch. The retired `fitsActualCap` is gone; a limit is never capped by CASA.

### Jar-to-jar balance transfer (implemented, plan `260923-jar-to-jar-transfer-sheet`)

Tapping a `JarEnvelopeCard` on Tổng quan, current month only, opens `JarTransferSheet` (`src/components/hu-envelope/JarTransferSheet.tsx` + `JarTransferEndpointRow.tsx`/`JarTransferPicker.tsx`/`JarTransferAmountField.tsx`) — replacing the earlier tap-to-navigate-to-Ngân-sách behaviour. It moves SỐ DƯ, never `budgetLimit`, between one source and one destination (a jar or the "Chưa phân bổ" pool). A past month's cards pass no `onOpen` handler and stay view-only.

- **Shared pure rules (`src/domain/engine/jar-transfer-rules.ts`)** — `transferEndpoints`/`transferCapOf`/`isValidTransferAmount`/`validateTransfer`/`previewTransfer` are the one place deciding who can give/receive how much; used by both `JarTransferSheet` and `AgentRebalanceCard` (DRY, invariant #1 — every number comes from the same engine snapshot, never invented by the UI or the LLM). A jar endpoint's `cap` is `max(0, balance)`, `null` when the jar has no balance yet (invariant #6, "Chưa có số dư" — it can then neither give nor receive); the pool endpoint's `balance` is the signed `computeUnallocatedPool(...).amount` (negative when over-allocated) and its `cap` is `max(0, amount)`.
- **Write is one existing `dieu-chinh-hu` leg, no new write door.** The sheet calls the existing `useAutoFundWith(...).commitPersisted` path with `origin: "manual"`, `triggerTxnId: jar-transfer-<Date.now()>`, and `targetJarId` set to the destination jar id (or `null` for the pool) — the same leg shape `AgentRebalanceCard`/`useAutoFund` already write (no new table, no new `Transaction` field).
- **Server guard (`src/app/api/manual-transactions/rebalance-leg-guard.ts`, `rebalanceLegViolation`)** re-checks every `dieu-chinh-hu` leg on `POST` and `PATCH /api/manual-transactions` (including an amount-only edit of an existing leg) — the client rules above are UX only:
  1. `fromJarId !== toJarId` on **every** leg (422); a non-pool end must be a jar that exists in the persona's config (404).
  2. **Only a self-made re-split is capped** — a leg whose `triggerTxnId` is NOT a real transaction of the `cif` (a `jar-transfer-…` id from this sheet, or an `agent-…` id from `AgentRebalanceCard`): the source may give at most `max(0, balance)` as of the leg's own `postedAt` (pool: `max(0, unallocated)`), re-derived server-side from `jarBalances`/`computeUnallocatedPool`, not trusted from the request; a destination jar whose balance is `null` is rejected (422).
  3. A leg whose `triggerTxnId` IS a real transaction (the auto-fund donor waterfall, `reconcile`) is never capped by this guard — those flows delete-and-rewrite legs the server hasn't yet seen the corrected category for, and a false reject there would strand the cover.
- **Known limitation:** `cif` is client-sent and not authenticated (pre-existing, prototype-wide) — this guard is a consistency check, not an authorization boundary.

On success, `HuOverviewRow` shows a `role="status"` banner ("Đã chuyển X từ A sang B") for 4 seconds; it does not mutate `spent`, `overLimit`, or any jar's `budgetLimit`.
...
- Jar balance vs limit (implemented — `src/domain/engine/__tests__/jar-balance.test.ts` cases A–L: funded, over limit but funded, overdrawn + pool cover, month rollover, mid-month anchor, unfunded `null`, withdraw, past-month cut-off, zero opening, template jar, pre-anchor month, one clock) and envelope identity (`jar-envelope.test.ts`, fixtures via `withSeedDeposits`): `pending.amount === pool − Σ max(0, balance)`, on fixtures — including the unfunded case (`balance`/spendable never fabricate a 0), the overspent (negative `balance`) case, and the no-current-account case (pool/pending stay "unknown"), plus a determinism check (calculation-correctness gate).
- Two independent axes (implemented, plan `260920-1317-jar-transfer-balance-not-limit` — `src/domain/engine/__tests__/jar-budget-guards.test.ts`, `jar-budget.test.ts`, `jar-envelope.test.ts`): a rebalance leg that fully or partially covers an overspent jar's `balance` never flips `status`/`overLimit` back to `"ok"`/`false` — `line.status === "over" ⇔ line.spent > line.limit` holds for every leg size in a sweep (`[0, 10k, 49_999, 50k, 50_001, 200k]`), independent of the balance axis (`jarNeedsManualTopUp(line.balance) === (line.balance < 0)`).
- **A negative jar balance is shown as negative (implemented, plan 260923 — `src/components/budget/__tests__/budget-tab.test.tsx`):** the balance is real money in the jar, so an overdrawn jar's "Số dư" reads negative in `text-negative` (the same figure on Tổng quan); only spendable (transfer source, pool) floors at 0. `ManualCoverNotice` carries the action ("đã tiêu quá số dư X chưa được bù", `X = −balance`, the BALANCE axis) and, separately, "Đã vượt hạn mức Y" keeps stating the PLAN axis (`Y = spent − limit`). `jar-pressure`'s fact flips label instead of sign (`Còn lại` → `Cần bù`, always a positive number) — asserted on `insight.sourceFacts` in `src/insights/__tests__/jar-pressure.test.ts`, since `factValues` abs-normalises and cannot see a sign regression.
- Cap and ledger tests (implemented — `src/domain/engine/__tests__/allocation-plan.test.ts` for `fitsCasaCap`; `src/app/api/jars/__tests__/route.test.ts`, `route-write-guards.test.ts` and `src/app/api/jar-ledger/__tests__/route.test.ts` for the write doors; `src/lib/__tests__/db-migrate-jar-ledger.test.ts` for the migration + backup; `src/domain/engine/__tests__/jar-pool-parity.test.ts` for D26): a write never raises Σ spendable past CASA, limit edits are never capped, withdraws never exceed the balance, the batch is all-or-nothing, and card/sheet/picker/cap read one pool number. (The former `actualAmount` H3 re-sync and its `fitsActualCap`/`jar-rules-resync` tests were removed with the field itself — a jar carries no stored balance to cap or re-sync anymore.)
- Derived-spendable, unallocated pool, and funding-tree tests (implemented — `src/domain/engine/__tests__/jar-budget.test.ts` for `jarSpendable`, `.../unallocated-pool.test.ts`, `.../jar-funding.test.ts`): `jarSpendable` is `max(0, balance)` and stays `null` for an unknown balance (never 0, invariant #6); `casaBalance` sums only `current` accounts across multiple such accounts (never `accounts[0]`); `computeUnallocatedPool({casaBalance, spendableTotal})` never clamps a negative pool (`overAllocated` true, true negative kept); `evaluateFunding` covers `ok`/`topup`/`insufficient` tiers over `JarSpendable[]`, the donor order (pool first, then jars largest-`spendable`-first, no protected jar), the `insufficient` residual no donor set can cover, `canDonate` filtering, multi-donor chains, and the pool-as-source direction (`sourceJarId: null`) where donors lift the pool with no self-donation and no target jar.
- Rebalance-as-Transaction fold (implemented — `src/domain/engine/__tests__/jar-rebalance.test.ts` for `rebalanceNetByJar`/`rebalanceTxns`): only posted, in-period, `REBALANCE_CATEGORY`-tagged txns with `rebalance` meta count; `Σ nhận − Σ cho` folds correctly per real jar; the `"pool"` end of a leg is never credited/debited as a bucket (never reintroduces `pool + Σ balance`); `spent` stays untouched by any rebalance (invariant #6).
