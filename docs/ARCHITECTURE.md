...
- **`src/domain/engine/jar-funding.ts` (`evaluateFunding({amount, sourceJarId, casaBalance, jars})`)** — the funding decision tree used when a chosen transfer source (a jar, or the pool itself when `sourceJarId` is `null`) doesn't hold enough. Pure and deterministic; the LLM is not involved anywhere in this module. Returns a `FundingAssessment`:
  ...
  - `tier: "topup"` — the source is short, but the pool plus other jars can cover the `shortfall`; `donors[]` lists the proposed chain (in take order) built by `buildDonorChain`: the pool first, then discretionary jars largest-`spendable`-first, then fixed jars largest-first as a last resort. Only jars that `canDonate` (a non-null `spendable` and at least one category to charge the take against) are candidates. A jar counts as fixed (`isJarFixed`) only when *every* one of its `categoryIds` is in `FIXED_CATEGORY_IDS` — `Jar` itself carries no `fixed` field, so "essential" is derived, not stored; a mixed jar is treated as discretionary so it stays tappable.
...
- **Top-up is applied as spend, exactly once, at `TransferConfirm.confirm()`** (`src/app/transfer-confirm/TransferConfirm.tsx`) — never at the popup. Before debiting, if the draft carries `plannedReallocation` or `overspend`, the confirm screen rebuilds the per-jar `spendable` figures from the **freshest** `financials.jarBudget.lines` and re-runs `evaluateFunding` against the current CASA balance and jar state (never the popup-time snapshot — state may have drifted between screens). `tier: "insufficient"` on re-check aborts with an error (no debit, no charge, latch released for retry); `tier: "ok"` means the state already improved and no top-up is needed; `tier: "topup"` applies the freshly-computed donor chain via **Approach A "spread-as-spend"**: it writes one self-reported expense transaction per contributing jar (`addManualTxn`) — the source jar's own portion `min(amount, spendable)` into its first category, plus each donor's `take` into that donor's first category — so recording the spend against each jar's category is what draws its derived balance down. The **pool** portion needs no transaction: the outward account debit already shrinks the derived pool. `budgetLimit` is **never** mutated (invariant #5: no silent hạn-mức change). Cancelling the confirm screen (navigating away without tapping "Xác nhận") writes nothing — the top-up is part of this single confirm action.
- **Why spend, not a balance transfer.** Because a jar's spendable is now derived (`max(0, budgetLimit − spent)`), there is no stored jar balance to move between labels; the only lever that changes a jar's derived spendable is its recorded spend. Charging each contributor its portion as a `self_reported` expense is therefore both the honest bookkeeping (the money really was spent toward the outward transfer) and the mechanism that rebalances the derived numbers — no separate ledger, no `applyReallocation`.
