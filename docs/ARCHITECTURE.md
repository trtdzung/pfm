...
### Unallocated pool + funding decision tree (implemented, `PFM-025`/`PFM-146` follow-on)

Two small, pure engine additions on top of the jar model above, both feeding the Chuyển tiền (transfer) flow. Neither introduces a new stored field.
...
- **`src/domain/engine/jar-funding.ts` (`evaluateFunding({amount, sourceJarId, casaBalance, jars})`)** — the funding decision tree used when a chosen transfer source (a jar, or the pool itself when `sourceJarId` is `null`) doesn't hold enough. Pure and deterministic; the LLM is not involved anywhere in this module. Returns a `FundingAssessment`:
  ...
  - `tier: "topup"` — the source is short, but the pool plus other jars can cover the `shortfall`; `donors[]` lists the proposed chain (in take order) built by `buildDonorChain`: the pool first, then discretionary jars largest-`actualAmount`-first, then fixed jars largest-first as a last resort. A jar counts as fixed (`isJarFixed`) only when *every* one of its `categoryIds` is in `FIXED_CATEGORY_IDS` — `Jar` itself carries no `fixed` field, so "essential" is derived, not stored; a mixed jar is treated as discretionary so it stays tappable.
  - `tier: "insufficient"` — `amount` exceeds the whole CASA balance; hard block, empty `donors[]`, no draft is produced downstream.
  - When `sourceJarId` is `null` (a pool-sourced transfer), the roles invert: the shortfall is against the pool itself, donors are jars pulled down to lift the pool, there is no pool-as-its-own-donor and no `targetJarId`.
- **UI: `JarTopupSuggestionSheet`** (`src/components/transfer/JarTopupSuggestionSheet.tsx`), shown from `TransferCompose.tsx` when `evaluateFunding` returns `topup`. It only ever renders numbers straight from the `FundingAssessment` (invariant #1) and offers three choices — **Đồng ý rót** (attach the proposed donor chain to the draft as `plannedReallocation`), **Chọn nguồn khác** (go back and pick a different source), **Bỏ qua, vượt hũ** (proceed with `overspend: true` on the draft, no reallocation). None of the three touches a jar or account balance at this point — `TransferDraft`/`StoredTransferDraft` (`src/lib/transfer-draft-store.ts`) just carries the intent (`plannedReallocation?: {donors, targetJarId}` or `overspend?: boolean`) through to the confirm screen.
- **Reallocation is applied exactly once, atomically, at `TransferConfirm.confirm()`** (`src/app/transfer-confirm/TransferConfirm.tsx`) — never at the popup. Before debiting, if the draft carries `plannedReallocation` or `overspend`, the confirm screen re-fetches accounts and re-runs `evaluateFunding` against the **freshest** CASA balance and jar state (never the popup-time snapshot — state may have drifted between screens). `tier: "insufficient"` on re-check aborts with an error (no debit, no reallocation, latch released for retry); `tier: "ok"` means the state already improved and no reallocation is needed; `tier: "topup"` applies the freshly-computed donor chain via `applyReallocation` before the account debit and the jar spend. Cancelling the confirm screen (navigating away without tapping "Xác nhận") leaves every jar/account untouched — nothing is written until this single point.
- **`applyReallocation` (`src/state/jars.tsx`)** merges every donor decrement and the target credit into deltas first (a jar can appear as both a donor and, in a chain, indirectly affect the target), throws before any write if a donor would go negative, and then issues **one** batch `updateJars` call — never a pairwise loop, which could lose updates or vaporise money across concurrent writes. A "pool" donor produces no jar patch at all (the pool is derived, not a row); it simply isn't offset by a matching credit.
- **Server-side ceiling, independent of the client's proposal.** Both jar write routes (`PATCH /api/jars` batch and `PATCH /api/jars/:id` single, `src/app/api/jars/route.ts` / `.../[id]/route.ts`) call `fitsActualCap(jars, casaPool)` (`src/domain/engine/allocation-plan.ts`) whenever a patch touches `actualAmount`: it fails closed on an `"unknown"` CASA pool (invariant #6), rejects (`422`) if Σ `actualAmount` across the resulting jar set would exceed the CASA balance, and rejects if any jar's `actualAmount` would go negative. This is the same file/pattern as the existing `fitsCasaCap` check for Σ `budgetLimit` (see "Cap enforcement" above under the envelope model) — one guards the allocation/ceiling number, the other guards the spendable-balance number, and both are enforced on the server, not just as client UX.
...
- Unallocated pool and funding-tree tests (implemented — `src/domain/engine/__tests__/unallocated-pool.test.ts`, `src/domain/engine/__tests__/jar-funding.test.ts`): `casaBalance` sums only `current` accounts across multiple such accounts (never `accounts[0]`); `computeUnallocatedPool` never clamps a negative pool (`overAllocated` true, true negative kept); `evaluateFunding` covers `ok`/`topup`/`insufficient` tiers, donor priority (pool → discretionary largest-first → fixed last), multi-donor chains, and the pool-as-source direction (`sourceJarId: null`) where donors lift the pool with no self-donation and no target jar. `fitsActualCap` (`allocation-plan.test.ts`) covers Σ `actualAmount` ≤ CASA and the negative-balance rejection.
